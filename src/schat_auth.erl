...
%% API
-export([start_link/0, auth/2,process/1]).
...
process(Packet) ->
  AuthInfo = parse_packet(Packet#packet.body,#packet_auth{}),
  auth(AuthInfo#packet_auth.username,AuthInfo#packet_auth.password).
...
parse_packet([Item|Packet],P) ->
  case Item of
    {<<"username">>,UserName} ->
      NewP = P#packet_auth{username = UserName};
    {<<"password">>,PassWord} ->
      NewP = P#packet_auth{password = PassWord}
  end,
...
handle_call({auth, UserName, Password}, _From, State) ->
  case schat_odbc:get_user_by_up(UserName,Password) of
    [] ->
